...
@Controller
@SessionAttributes({ "frageformular", "frage" }) // gibt an welche Attribute auf Serverseite gespeichert und über
                                                 // mehrere Anfragen hinweg verfügbar sein sollen // hier wird das
                                                 // Attribut mit "${frageformular}" in der Sitzung beibehalten
public class FrageController {
    ...
    // Diese Methode wird jedes Mal aufgerufen, wenn der Controller eine Anforderung
    // empfängt
    // Dadurch können andere Methoden im Controller auf das FrageFormular-Objekt
    // zugreifen, indem sie den Parameter @ModelAttribute("frageformular")
    // FrageFormular formular in ihren Methodenaufrufen deklarieren
    @ModelAttribute("frageformular")
    public void initFrageFormular(Model m) { // ie Übermittlung von Daten zwischen der Controller-Schicht:
                                             // addAttribute() und der View-Schicht : th:text="${key}"
    ...
    // sortierte Liste aller Fragen nach Kategorie und Punktzahl zu erhalten und in
    // frageliste.html anzuzeigen
    @GetMapping("/frage")
    ...
    // die Frage mit der angegebenen ID wird gelöscht //"redirect:/frage" wird
    // zurückgegeben, um auf die Frageliste Seite umgeleitet zu werden
    ...
    // wenn eine GET-Anfrage an die URL "/frage/{fragenr}" gesendet wird, wird die
    // Methode aufgerufen
    @GetMapping("/frage/{fragenr}")
    public String anzeigen(@PathVariable int fragenr, // @PathVariable um eine Variable aus der URL-Anfrage zu
                                                      // extrahieren und sie als Methodeparameter zu übergeben
            Model m,
        ...
        if (fragenr == 0) {
            ...
            m.addAttribute("frageformular", formular); // frageformular-Session-Attribut wird mit einer neuen leeren
                                                       // Instanz von FrageFormular initialisiert
            m.addAttribute("frage", new Frage()); // neue leere Frage //frage-Session-Attribut wird mit einer neuen
                                                  // leeren Instanz von Frage initialisiert
        }
    ...
    // Verarbeitungsmethode für eine HTTP-POST-Anfrage an die URL "/frage/{fragenr}"
    // //kann in der Anfrage enthaltene Variable zugreifen
    ...
    public String formular_post(@PathVariable int fragenr,
            ...
            @ModelAttribute("frage") Frage frage,
            @RequestParam(required = false) String neuerEintrag) { // @RequestParam(required = false) gibt an, dass der
                                                                   // Parameter nicht erforderlich ist//Wenn der
                                                                   // Parameter in der Anfrage enthalten ist, wird er an
                                                                   // den Controller-Methodenparameter gebunden // view:
                                                                   // name="name"
        ...
        // prüft, ob die Anzahl der falschen Antworten im Frageformular nicht bereits
        // das maximale Limit erreicht hat
        if (formular.getFalscheAntworten().size() != MAX_FALSCH) {
            if (neuerEintrag != null && !neuerEintrag.isEmpty()) { // prüft, ob die Variable neuerEintrag einen Wert
                                                                   // hat, der nicht null und nicht leer ist
        ...
        if (formularErrors.hasErrors()) {
            ...
            try {
                // gespeicherteFrage in das Session-Attribut frage gespeichert, um
                // sicherzustellen, dass es immer die aktuellste Version enthält
                Frage gespeicherteFrage = frageService.speichereFrage(frage);
                m.addAttribute("frage", gespeicherteFrage);
                if (fragenr == 0) {
                    // leitet den Nutzer per Redirect auf die "korrekte" URL /frage/id weiter, um
                    // nicht auf der Neuanlegen-URI /frage/0 zu bleiben
