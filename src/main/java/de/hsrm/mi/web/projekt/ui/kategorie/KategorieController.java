...
public class KategorieController {
    ...
    // mit der @ModelAttribute-Annotation wird Kategorie initialisiert und in der
    // Sitzung beibehalten
    ...
    // Wenn diese URL aufgerufen wird, werden alle Kategorien aus der Datenbank
    // abgerufen und dem Model hinzugefügt.
    @GetMapping("/kategorie")
    public String kategorienListe(Model m) {
        List<Kategorie> kategorien = kategorieService.holeAlleKategorien(); // Die Liste der Kategorien wird dem Model
                                                                            // mit dem Attributsnamen "kategorien"
                                                                            // hinzugefügt
    ...
    public String deleteKategorie(@PathVariable("id") Long id, Model m) throws AccessDeniedException {
    ...
    // Wert der {id} aus der URL-Anfrage wird extrahiert und kann in der Methode
    ...
    public String anzeigen(@PathVariable long id,
        ...
        if (id == 0) {
            formular = new KategorieFormular(); // Neue leere KategorieFormular wird erstellt und der Variable
                                                // "formular" zugewiesen
            m.addAttribute("kategorieFormular", formular); // "formular" wird als Attribut mit dem Namen
                                                           // "kategorieFormular" dem Model m hinzugefügt. Damit kann
                                                           // das Formular in der View verwendet werden.
            m.addAttribute("kategorie", new Kategorie()); // Neue leere Kategorie wird erstellt und mit dem Namen
                                                          // "kategorie" dem Model m hinzugefügt
        }

        if (id > 0) {
            Optional<Kategorie> optionalKategorie = kategorieService.holeKategorieMitId(id); // die Kategorie mit der
                                                                                             // angegebenen id wird in
                                                                                             // einem Optional-Typ
                                                                                             // "optionalKategorie"
                                                                                             // gespeichert.
            if (optionalKategorie.isPresent()) { // Es wird überprüft, ob die optionalKategorie (mit der angegebenen id)
                                                 // existiert
                kategorie = optionalKategorie.get(); // Der Wert wird geholt und der Variable kategorie zugewiesen
                formular.fromKategorie(kategorie); // Die Daten aus der kategorie werden über die Methode
                                                   // fromKategorie(kategorie) in das KategorieFormular übertragen
                m.addAttribute("kategorie", kategorie); // Die Kategorie wird als Attribut mit dem Namen "kategorie" dem
                                                        // Model m hinzugefügt
            }
        }
